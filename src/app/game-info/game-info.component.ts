...
export class GameInfoComponent implements OnChanges{
  ...
  ngOnChanges(): void {
    if(this.card){
    console.log('current card:', this.card);         // methode wird jedes mal ausgeführt wenn sich was ändert, void bedeutet das es was ausgeführt wird aber wird nichts zuruck gegeben.
    let cardNumber = +this.card.split('_')[1];       // mit methode split kann ich ein string belibig aufteilen mit 1 sagen wir was sollte rausgegeben werden aus dem geschafenen array, mit plus machen wir es zum number
